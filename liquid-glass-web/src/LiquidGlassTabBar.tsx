...
export function LiquidGlassTabBar({
  ...
  const cellWidth = () => (BAR_WIDTH - BAR_INSET * 2) / Math.max(1, items.length);
  const itemCenter = (index: number) => BAR_INSET + cellWidth() * (index + .5);

  const syncBase = (stretch: number, pressed: boolean) => {
    baseRendererRef.current?.setGeometry(BAR_WIDTH / 2, BAR_HEIGHT / 2, stretch, pressed, 1, 1, 0);
  };
  ...
  useEffect(() => {
    ...
    try {
      const baseSettings = { ...mergedSettings, lensWidth: BAR_WIDTH - 4, lensHeight: BAR_HEIGHT - 8, radius: 32 };
      ...
      baseRenderer.resize(BAR_WIDTH, BAR_HEIGHT);
      indicatorRenderer.resize(BAR_WIDTH, BAR_HEIGHT);
